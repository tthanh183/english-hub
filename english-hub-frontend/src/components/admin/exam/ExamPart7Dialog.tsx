...
type ExamPart7DialogProps = {
  examId?: string;
  question?: QuestionResponse;
  onClose: () => void;
};

export default function ExamPart7Dialog({
  examId,
  question,
  onClose,
}: ExamPart7DialogProps) {
  ...
  const handleUpdateQuestions = async (data: {
    ...
    if (data.questionData.length > sortedQuestions.length) {
      // Có thêm câu hỏi mới: update câu cũ và add câu mới
      ...
      const addResult = await addQuestionsToExam(data.examId, newQuestions);
